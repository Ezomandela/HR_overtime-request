...
class HrOverTime(models.Model):
    ...
    overtime_type=fields.Many2one('overtime.type', string="OverType")
        ...
        for sheet in self:
            if sheet.date_from and sheet.date_to:
                ...
                if hours<=8:
                    ...
                    raise ValidationError(' the valid hour must less than or equals to 8:00hours ')
    ...
    def approve(self):
        if self.overtime_type_id.type == 'Night' or  self.overtime_type_id.type == 'Normal' or self.overtime_type_id.type == 'Holiday'  :
...
class HrOverTimeType(models.Model):
    ...
    type= fields.Selection([('Night', 'night'),('Normal', 'normal'),('Holiday', 'holiday')] ,default="Night", required=True)
    duration_type = fields.Selection([('hours', 'Hour')], string="Duration Type", default="hours",
                                     required=True)
